<template>
  <ion-menu side="start" content-id="menu" menu-id="main">
    ...
    <ion-content>
      <ion-item>
        <router-link to="/" @click="closeMainMenu">
          Главная
        </router-link>
      </ion-item>
      <ion-segment class="segment" value="Женщины">
        <ion-segment-button class="segment__button" v-for="(gender, i) in genders" :key="i" :tab="gender" :value="gender">
          <ion-label class="segment__label">{{ gender }}</ion-label>
        </ion-segment-button>
      </ion-segment>
      <ion-accordion-group :multiple="true">
        <ion-accordion class="accordion" v-for="accordion in accordions" :key="accordion.id" :value="accordion.title">
          <ion-item class="accordion__item" slot="header" color="transparent" lines="none">
            <ion-label class="accordion__label">{{
              accordion.title
            }}</ion-label>
          </ion-item>
          <ion-list class="accordion__list" slot="content">
            <router-link class="accordion__link" :to="`#${item}`" v-for="(item, idx) in accordion.list" :key="idx">{{ item
            }}</router-link>
          </ion-list>
        </ion-accordion>
      </ion-accordion-group>
      <address class="contacts ion-padding">
        <div class="contacts__item">
          <icon-phone />
          <router-link to="#" class="contacts__link">+7(800)-555-35-35</router-link>
        </div>
        <div class="contacts__item">
          <icon-location-pin />
      ...
      <div class="menu-footer">
        ...
        <div class="socials">
          <router-link class="socials__link" to="/#"><icon-twitter-dark /></router-link>
          <router-link class="socials__link" to="/#"><icon-instagram-dark /></router-link>
          <router-link class="socials__link" to="/#"><icon-youtube-dark /></router-link>
        </div>
      </div>
    </ion-content>
  </ion-menu>
